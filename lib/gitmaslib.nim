...
proc YLW*(): string {.exportc, dynlib.} = 
  ansiStyleCode(styleBright) & ansiForegroundColorCode(fgYellow)

proc RST*(): string {.exportc, dynlib.} = 
  ansiResetCode

proc RED*(): string {.exportc, dynlib.} = 
  ansiStyleCode(styleBright) & ansiForegroundColorCode(fgRed)
...
proc doSetup*() {.exportc, dynlib.} =
  ...
  if token.len > 0:
    try:
      ...
      setFilePermissions(tokenPath, {fpUserRead, fpUserWrite})
